::  books-proxy-hook: make local pages available to foreign ships
::
...
::
/-  group-store, *metadata-store, *books-store
...
=<
  ...
  ++  on-watch
    |=  =path
    ^-  (quip card _this)
    ::  the local ship should just use books-store directly
...
++  permitted
  |=  [who=ship =path]
  ^-  ?
  ::  we only expose /local-books and /annotations,
  ...
  %+  lien  (groups-from-resource:md %books u.target)
  |=  =group-path
  ^-  ?
...
::
::  groups subscription
::TODO  largely copied from books-listen-hook. maybe make a store-listener lib?
::
++  watch-groups
  ^-  card
...
++  handle-group-update
  ...
  :_  state
  ?.  ?=(%remove -.upd)  ~
  ::  if someone was removed from a group, find all books resources associated
  ::  with that group, then kick their subscriptions if they're no longer
  ::
  %-  zing
  %+  turn  (app-paths-from-group:md %books pax.upd)
  |=  =app-path
  ^-  (list card)
  %+  kick-revoked-permissions
    app-path
...
++  handle-proxy-sign
  ...
  ?-  -.sign
    ...
    %kick         [[(proxy-pass-books-store wire %watch wire)]~ state]
  ::
      %watch-ack
    ?~  p.sign  [~ state]
    =/  =tank
      :-  %leaf
      "{(trip dap.bowl)} failed subscribe to books-store. very wrong!"
    %-  (slog tank u.p.sign)
    [~ state]
  ==
::
++  proxy-pass-books-store
...
++  initial-response
  ...
  ^-  card
  =;  =initial
    [%give %fact ~ %books-initial !>(initial)]
  ?+  path  !!
      [%local-books ^]
    [%local-books (scry-for (map ^path books) %books-store path)]
  ::
      [%annotations %$ ^]
    [%annotations (scry-for (per-path-url notes) %books-store path)]
  ==
::
++  start-proxy
  |=  [who=ship =path]
  ...
  ::
  (proxy-pass-books-store path %watch path)
::
++  stop-proxy
  |=  [who=ship =path]
  ^-  (quip card _state)
  ...
  ::
  [(proxy-pass-books-store path %leave ~)]~
::
::  helpers
::
++  scry-for
